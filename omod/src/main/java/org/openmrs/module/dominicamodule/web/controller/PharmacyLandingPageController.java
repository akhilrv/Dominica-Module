...
public class PharmacyLandingPageController {
    ...
    protected ModelAndView getForm(@RequestParam(value = "orderId") Integer orderId){
        ...
        Integer patientId=Context.getOrderService().getOrder(orderId).getPatient().getId();
        String drugConcept=Context.getOrderService().getOrder(orderId).getConcept().getName().getName();
        String allergies=getAllergensForPatient(patientId);
        String allergyAlert=new String();
        if (allergies.contains(drugConcept)){
            allergyAlert="*** Patient Allergic to prescribed drug : "+drugConcept+" ***";
        }
        else{
            allergyAlert="";
        }
        mav.addObject("allergies",allergyAlert );
    ...
    private void removeFilledOrders() {
        ...
        for (DrugOrder order : allOrders) {
            ...
            if (dos == DrugOrderStatus.APPROVED || dos == DrugOrderStatus.NOAPPROVALNEEDED || dos == DrugOrderStatus.RENEWEDNOAPPROVALNEEDED || dos == DrugOrderStatus.RENEWEDAPPROVED) {
                orders.add(order);
